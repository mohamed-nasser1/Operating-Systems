...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	int SEM_id = get_semaphore_object_ID(ownerEnvID,semaphoreName);
	if(SEM_id == E_SEMAPHORE_NOT_EXISTS)
	{
	  SEM_id = allocate_semaphore_object(&SEM);
	  if(SEM_id!=E_NO_SEMAPHORE)
	  {
		 // semaphores[SEM_id].env_queue.size++; ////////////
		  semaphores[SEM_id].ownerID=ownerEnvID;
		  strcpy(semaphores[SEM_id].name,semaphoreName);
		  semaphores[SEM_id].value=initialValue;
		  return SEM_id;
	  }
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	int SEM_id = get_semaphore_object_ID(ownerEnvID,semaphoreName);
	//struct Semaphore *SEM=NULL;//semaphores[SEM_id];
	semaphores[SEM_id].value--;
	if(semaphores[SEM_id].value < 0)
	{
		//sched_remove_ready(myenv);
		enqueue(&(semaphores[SEM_id].env_queue),myenv); /*enqueue this env to que of blocked envs*/
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	if(semaphores[SEM_id].value <= 0)
	{
		struct Env* myenv =dequeue(&(semaphores[SEM_id].env_queue));
